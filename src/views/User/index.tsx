...
import { BOARD_PATH, BOARD_WRITH_PATH, MAIN_PATH, USER_PATH } from 'constant';
...
export default function User() {
  ...
  const UserBottom = () => {
    ...
    const onSideCardClickHandler = () => {
      if (isMyPage) navigate(BOARD_PATH() + '/' + BOARD_WRITH_PATH());
      else if (loginUser) navigate(USER_PATH(loginUser.email));
    }

    // effect : user email path variable 이 변경 될 때마다 실행할 함수
    useEffect(() => {
      if (!userEmail) return;
