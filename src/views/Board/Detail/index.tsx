...
import { deleteBoardRequest, getBoardRequest, getCommentListRequest, getFavoriteListRequest, incresaeViewCountRequest, postCommentRequest, putFavoriteRequest } from 'apis';
...
export default function BoardDetail() {
  ...
  const incresaeViewCountResponse = (responseBody: IncreaseViewCountResponseDto | ResponseDto | null) => {
  ...
  const BoardDetailBottom = () => {
    ...
    // effect : 게시물 번호 path variable 이 바뀔때 마다 좋아요 및 댓글 리스트 불러오기
    useEffect(() => {
      if (!boardNumber) return;
  ...
  // effect : 게시물 번호 path variable 이 바뀔때 마다 게시물 조회수 증가
  let effectFlag = true;
  useEffect(() => {
    if (!boardNumber) return;
    if (effectFlag) {
    ...
    incresaeViewCountRequest(boardNumber).then(incresaeViewCountResponse);
